...
with DAG(
        dag_id="lesson_3_example_sensors_v1",
        schedule=None,
        start_date=pendulum.datetime(2021, 1, 1, tz="UTC"),
        catchup=False,
        tags=["example", "lesson_3"],
) as dag:
    ...
    t4 = BashSensor(task_id="Sensor_fails_after_3_seconds",
                    timeout=3, soft_fail=True,
                    bash_command="exit 1")

    # [END example_bash_sensors]

    t5 = BashOperator(task_id="remove_file",
                      bash_command="rm -rf /tmp/temporary_file_for_testing")

    # [START example_file_sensor]

    t6 = FileSensor(task_id="wait_for_file",
    ...
    t7_check = BashOperator(
        task_id="check_create_file_after_3_seconds",
        bash_command="ls /tmp/temporary_file_for_testing",
        do_xcom_push=True
    )

    t7_sensor = FileSensor(task_id="wait_for_file_create_file_after_3_seconds",
                           filepath="/tmp/temporary_file_for_testing")

    t7 >> t7_check >> t7_sensor
